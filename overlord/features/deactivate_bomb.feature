Feature: Deactivate the bomb
  ...
  Scenario: Bomb deactivates with default deactivation code (happy)
  	When I enter the default deactivation code
  	Then the bomb is deactivated

  Scenario: Bomb does not deactivate after one incorrect deactivation attempt (sad)
  	When I enter an incorrect deactivation code
    And I try to deactivate the bomb
  	Then the bomb does not deactivate
  	And there are 2 deactivation attempts remaining

  Scenario: Bomb does not deactivate after two incorrect deactivation attempts (sad)
